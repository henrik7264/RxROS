...
int main(int argc, char** argv)
{
    ...
    //const auto keyboardDevice = rxros::Parameter::get("/keyboard_publisher/device", "/dev/input/event1"); // for Dell PC
    const auto keyboardDevice = rxros::Parameter::get("/keyboard_publisher/device", "/dev/input/event4"); // for other PCs

    rxros::Logging().info() << "Keyboard device: " << keyboardDevice;

    auto keyboardEventToKeyboardMsg = [](auto keyboardEvent) {
